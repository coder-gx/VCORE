...
'name=qwen3_4b_math_vcore_single   model_name_or_path=/home/data-1/model_base/Qwen3-4B lora_rank=8 lora_alpha=16 lora_dropout=0.1 dataset=math_sft template=qwen3 output_dir=/home/code/VCORE/output/math/qwen3_4b_vcore/lora/sft save_steps=100 gradient_accumulation_steps=8 learning_rate=2e-5 warmup_ratio=0.1 use_vcore=False vcore_temperature=5e3 vcore_anchor_steps=1 vcore_epsilon=1e-4 vcore_pre_ga=8 vcore_single_process=True'
...
for cfg in "${EXPS[@]}"; do
  # CLEAR VARIABLES
  unset name model_name_or_path lora_rank lora_alpha lora_dropout dataset template output_dir save_steps learning_rate warmup_ratio use_dft use_vcore vcore_temperature vcore_anchor_steps  vcore_epsilon  vcore_pre_ga gradient_accumulation_steps main_or_branch
  eval "$cfg"
